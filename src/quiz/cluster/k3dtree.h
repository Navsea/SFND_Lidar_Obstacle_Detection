...
struct KdTree
{
	...
	void searchHelper(std::vector<float> target, Node* node, uint depth, float distanceTol, std::vector<int>& ids)
	{
		if (node != NULL)
		{
			...
			if( (node->point[0]-distanceTol <= target[0])&&(node->point[0]+distanceTol >= target[0]) && (node->point[1]-distanceTol <= target[1])&&(node->point[1]+distanceTol >= target[1]) && (node->point[2]-distanceTol <= target[2])&&(node->point[2]+distanceTol >= target[2]))
			{
				// calculate the distance
				float distance = sqrt(pow(fabs(node->point[0]-target[0]),2) + pow(fabs(node->point[1]-target[1]), 2) + pow(fabs(node->point[2]-target[2]), 2));
